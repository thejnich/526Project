...
The php version is forked from the gpgAuth project, and works in conjunction with the gpgAuth Chrome browser extension.  Though we've got these tools working, we are still in the process of learning how they work so that we can change the implementation to fit our specifications. 
\subsection{Perl} \label{subsec:perl}
The perl implementation is running and can detect a user's input.
