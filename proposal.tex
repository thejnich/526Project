...
\begin{enumerate}
	...
	\item Whenever the client wants to authenticate with the content server, the client sends the email address corresponding to their public key to the content server, the content server remotely fetches the public key corresponding to the email address and does a challenge response with the client.
	\item The client decodes the challenge response and sends back the answer to the server.
